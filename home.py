...
if writeFilm == 1:
    ...
    cols = st.columns(2)
    with cols[0]:
#        for id in algoIndex[0][1:]:
#            makeAffichage(id)
#            st.write("---")
        makeAffichage(algoIndex[0][1], level=1, contain=False)
        st.write("---")
    with cols[1]:
        makeAffichage(algoIndex[0][2], level=1, contain=False)
        st.write("---")
    with cols[0]:
        makeAffichage(algoIndex[0][3], level=1, contain=False)
        st.write("---")
    with cols[1]:
        makeAffichage(algoIndex[0][4], level=1, contain=False)
        st.write("---")
